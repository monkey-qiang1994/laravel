...
<div class="page-container">
  ...
  <span class="l">
    <a href="javascript:;" onClick="modaldemo()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加属性分类</a>
    <a href="javascript:history.go(-1);" class="btn btn-success radius"><i class="Hui-iconfont">&#xe66b;</i> 返回</a>
  </span>
  <span class="r">共有数据：<strong>{{count($att_list)}}</strong> 条属性</span>
  </div>
  ...
  <table class="table table-border table-bordered table-bg">
    <thead>
      <tr>
        <th scope="col" colspan="9">属性分类列表</th>
      </tr>
      <tr class="text-c">
        <th width="40">属性分类ID</th>
        <th width="150">属性分类名称</th>
        <th width="100">操作</th>
      </tr>
    </thead>
    <tbody>
    ...
    @foreach($att_list as $row)
      <tr class="text-c">
        ...
        <td>{{$row->cate_att_name}}</td>
        <td class="td-manage">
          <a title="查看属性" href="product_attribute?cate_att_id={{$row->cate_att_id}}" class="btn btn-success radius"><i class="Hui-iconfont">&#xe709;</i></a>
          <a title="编辑" href="javascript:void(0)" class="btn btn-primary radius edit"><i class="Hui-iconfont">&#xe6df;</i></a>
          <form action="product_attribute/{{$row->cate_att_id}}" method="post" style="display: initial;">
          {{ csrf_field() }}
          {{ method_field('DELETE') }}
  ...
  <div class="modal-dialog">
    <div class="modal-content radius">
      <div class="modal-header">
        <h3 class="modal-title">添加属性分类</h3>
        <a class="close" data-dismiss="modal" aria-hidden="true" href="javascript:void();">×</a>
      </div>
      <form action="product_attribute" method="post">
        <div class="modal-body">

          分类名称: <input type="text" name="cate_att_name" class="input-text radius" style="width:80%"><br/><br/>
  ...
  <div class="modal-dialog">
    <div class="modal-content radius">
      ...
      <form action="product_attribute/1" method="post">
        <input type="hidden" name="cate_att_id">
        <div class="modal-body">
          属性分类名称: <input type="text" name="cate_att_name" class="input-text radius edit_name" style="width:80%"><br/><br/>
