				...
				<div class="user-content__box clearfix bgf">
					...
					<div class="modify_div">
						<form action="/user/process_evaluation" method="post" class="evaluate-form__box" enctype="multipart/form-data">
							{{csrf_field()}}
							<input type="hidden" name="product_id[] " value="{{$detail_v->product_id}}">
							<input type="hidden" name="order_id" value="{{$detail_v->order_id}}">
							<span class="help-block">商品名称:{{$detail_v->product_name}}</span>
							<table class="table table-bordered">
								<tbody><tr>
									<th scope="row">评价等级</th>
									<td class="user-form-group fz0">
										<label><input name="grade{{$detail_v->product_id}}" value="0" checked="" type="radio"><i class="iconfont icon-radio"></i> <span>好评</span></label>
										<label><input name="grade{{$detail_v->product_id}}" value="1" type="radio"><i class="iconfont icon-radio"></i> <span>中评</span></label>
										<label><input name="grade{{$detail_v->product_id}}" value="2" type="radio"><i class="iconfont icon-radio"></i> <span>差评</span></label>
									</td>
								</tr>
								<tr>
									<th scope="row">评价商品</th>
									<td><textarea name="content{{$detail_v->product_id}}" rows="5" placeholder="请输入您对该商品的评价~"></textarea></td>
								</tr>
								<tr valign="middle">
									<th scope="row">晒图片</th>
									<td>
